...
create table if not exists public.sale_day_payments (
  id uuid primary key default gen_random_uuid(),
  warehouse_id uuid not null references public.warehouses(id) on update cascade on delete cascade,
...
create index if not exists idx_sale_day_payments_warehouse_date on public.sale_day_payments (warehouse_id, sale_date);
